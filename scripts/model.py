...
class Model(nn.Module):
    def __init__(self, n_channel, n_band,n_class = 2,PSD_en = True, PLV_en = True):
        super(Model, self).__init__()
        self.PSD_en = PSD_en
        self.PLV_en = PLV_en
        # 图神经网络部分
        assert PLV_en + PSD_en > 0, '必须使用至少一个特征'
        if PSD_en:
        ...
        # 后续的全连接层
        self.fc1 = nn.Linear(2 * n_channel * (PSD_en + PLV_en), 2 * n_channel)
